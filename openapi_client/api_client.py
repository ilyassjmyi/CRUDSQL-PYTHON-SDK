...
class ApiClient:
    ...
    def response_deserialize(
        ...
        try:
            ...
            # If response type is QueryFilterResponse, handle both single and multiple entities
            if response_type == "QueryFilterResponse":
                # Check if the response is already in the expected format
                if isinstance(json_data, dict) and all(
                    key in json_data
                    for key in ["data", "page", "page_size", "total", "total_pages"]
                ):
                    return_data = self.deserialize(
                        response_text, response_type, content_type
                    )
                else:
                    # Pass the data directly - our EntityData model will handle validation
                    wrapped_data = {
                        "data": json_data,
                        "page": 1,
                        "page_size": (
                        ...
                        "total": len(json_data) if isinstance(json_data, list) else 1,
                        "total_pages": 1,
                    }
                    return_data = self.deserialize(
                        json.dumps(wrapped_data), response_type, content_type
                    )
            else:
                return_data = self.deserialize(
                    response_text, response_type, content_type
